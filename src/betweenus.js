var BetweenUsModule = (function() {
  ...
  var _type_assert = function(type, expected_type) {
    ...
    if (Buffer.isBuffer(expected_type)) {
      if (!Buffer.isBuffer(type)) {
        throw new Error('Type Error. Expected: Buffer, Received: ' +
          typeof type + '.');
      } else {
        return true;
      }
    } else if (typeof type != typeof expected_type) {
      throw new Error('Type Error. Expected: ' + typeof expected_type +
  ...
  var GenerateSymmetricKeyDictionary = function() {
    ...
    var initialization_vector = _crypto.randomBytes(defaults.symmetric_algorithm
      .init_vector_byte_size);
   ...
   */
  var SymmetricDecrypt = function(encrypted_message,
    symmetric_key_dictionary) {
    _type_assert(encrypted_message, _types.buffer);
    _type_assert(symmetric_key_dictionary, _types.string);
    var key_object = _StringToKeyDataDictionary(symmetric_key_dictionary);
    var decipher = _crypto.createDecipheriv(defaults.symmetric_algorithm.algorithm_name,
      key_object.key, key_object.iv);
   ...
   */
  var SerializedDictionaryToShares = function(serialized_dictionary, shares,
    threshold, zeropadding) {
    ...
    var share_list = _secrets.share(_secrets.str2hex(
      serialized_dictionary), shares, threshold, zeropadding);
    return share_list;
  };
