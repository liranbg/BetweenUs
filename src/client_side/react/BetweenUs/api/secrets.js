...
(function (root, factory) {
    ...
    if (typeof define === "function" && define.amd) {
        ...
        // like Node.
        var crypto_extend = require("crypto-js");
        crypto_extend.randomBytesAsy = require('react-native-randombytes').randomBytes;
        module.exports = factory(crypto_extend);
    ...
    function reset() {
        ...
        CSPRNGTypes = ["asyncRandomBytes","nodeCryptoRandomBytes", "browserCryptoGetRandomValues", "browserSJCLRandom", "testRandom"];
    }
    ...
    function getRNG(type) {
        ...
        //Return a promise when has random bytes
        function asyncRandomBytes(bits) {
            var buf,
                bytes,
                radix,
                size;
            ...
            return new Promise(function(resolve, reject) {
                buf = crypto.randomBytesAsy(bytes, function(err, data) {
                    if (!err) {
                        resolve(construct(bits, data.toString("hex"), radix, size));
                    }
                    else {
                        reject(err);
                    }
        ...
        } else if (type && type === "asyncRandomBytes") {
            config.typeCSPRNG = type;
            return asyncRandomBytes;
    ...
    function getShares(secret, numShares, threshold, async) {
        var shares = [],
            coeffs = [secret],
            i,
            len;
        ...
        if (async) {
            ...
            return new Promise(function(f_resolve, f_reject){
                promiseWhile(
                    function() { return i < threshold; },
                    function() {
                        return new Promise(function(resolve, reject) {
                            config.rng(config.bits).then((result)=> {
                                coeffs[i] = parseInt(result, 2);
                                ++i;
                                resolve();
                            });
                        });
    ...
    var secrets = {
        ...
        setRNG: function (rng) {
            ...
            if (runCSPRNGTest) {
                ...
                if ((rng && typeof rng(config.bits) !== "string") && (typeof rng(config.bits).then !== "function")) {
                    throw new Error(errPrefix + "(Output is not a string)." + errSuffix);
                }

                if ((rng && !parseInt(rng(config.bits), 2)) && (typeof rng(config.bits).then !== "function")) {
                    throw new Error(errPrefix + "(Binary string output not parseable to an Integer)." + errSuffix);
                }

                if ((rng && rng(config.bits).length > config.bits) && (typeof rng(config.bits).then !== "function")) {
        ...
        random: function (bits) {
            ...
            if (config.typeCSPRNG === "asyncRandomBytes") {
                return new Promise(function(resolve, reject){
                    config.rng(bits).then((result) => {
                        resolve(bin2hex(result));
                    })
                });
            }
            else {
        ...
        share: function (secret, numShares, threshold, padLength, asyn) {
            var neededBits,
                subShares,
            ...
            if (asyn) {
                ...
                return new Promise(function(f_resolve, f_reject){
                    promiseWhile(
                        ...
                        function() {
                            return new Promise(function(resolve, reject) {
                                getShares(secret[i], numShares, threshold, asyn).then((subShares)=> {
